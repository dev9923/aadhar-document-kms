...
import type { WizardStepOrder } from "@/config/wizard-config";
import {
  computePath,
  getNextId,
  getPrevId,
...
function getDisplayStepIndex(
  ...
  if (
    currentId === "enrolmentType" ||
    currentId === "availableDocuments" ||
    currentId === "updateDocuments"
  )
    return 3;
  return 4; // results
}
...
export function DocumentChecker({
  ...
  const handleNext = (data: Record<string, string>) => {
    ...
    const cleaned = key
      ? resetFollowingSteps(updated, key as WizardStepOrder[number])
      : updated;
    setFormData(cleaned);
        ...
        {withIntro && (
          <div className="text-center space-y-3">
            <h2 className="text-3xl font-bold tracking-tight">
              Aadhaar Document Requirement Checker
            </h2>
            <p className="text-muted-foreground text-lg leading-snug text-balance">
              Review your tailored Aadhaar document checklist and download a copy
              for your visit.
            </p>
          </div>
        )}
  ...
  const initialMulti =
    step.type === "checkbox" ? parseMulti(formData[step.id]) : undefined;
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      {withIntro && (
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground text-lg leading-snug text-balance">
            Find out exactly which documents you need for Aadhaar enrolment or
            update, tailored to your age and situation.
          </p>
        </div>
      )}
